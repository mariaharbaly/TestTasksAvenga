...
namespace Refactoring
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            var price = 0m;
            StringBuilder orderSummary = new StringBuilder("ORDER SUMMARY FOR " + order.Item1 + ": \r\n");
            ...
            foreach (var orderProduct in order.Item2)
            {
                ...
                if (orderProduct.PricingMethod == "PerPound")
                {
                    ...
                    price += productPrice;
                    orderSummary.Append(" $" + productPrice + " (" + orderProduct.Weight + " pounds at $" + orderProduct.Price + " per pound)");
                }
                else // Per item
                {
                    productPrice = (orderProduct.Quantity.Value * orderProduct.Price);
                    price += productPrice;
                    orderSummary.Append(" $" + productPrice + " (" + orderProduct.Quantity + " items at $" + orderProduct.Price + " each)");
                }
